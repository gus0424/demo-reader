function CitedRefBlocks(A) {
    var e = jQuery;
    var r = 50;
    var E = 4000;
    var j = e("div.pmc_para_cit");
    var R = j.length;
    var h = e("div.portlet").not("div.pmc_para_cit");
    var P = o(); h.find("a.portlet_shutter").click(g);
    var x = -1;
    var p;
    var K = null;
    j.find("li").hoverIntent({ over: l, out: a, sensitivity: 1, interval: 150 });
    j.mouseover(q); j.mouseleave(C);
    var J = 0;
    var f = 3;
    var M = e("a.bibr[rid]");
    var Q = M.length; var I = 0;
    var w = 10;
    var N = e("#body-link-poppers");
    var O = {}; D(); function D() {
        for (var T = 0; T < f; ++T) {
            if (J >= R) {
                break;
    ...
    function F() {
        e("a.bibr[rid]").hoverIntent({ over: S, out: m, sensitivity: 1, interval: 150 })
    } function i(ai) {
        var ah = ai.attr("rid"); var U = e("#" + ah);
        if (U.length != 1) {
            ai.hide(); ai.data("myTop", 0);
            return
        }

        ai.data("myParagraph", U);
        var ac = parseInt(U.position().top) + parseInt(U.css("marginTop")) - 7;
        ai.data("myTop", ac);
        var ad = ai.find("li");
        ad.each(function () {
            var ak = e(this);
            var al = ak.height();
            var ao = ak.attr("reference_id");
            var an = U.find('a.bibr[rid="' + ao + '"]');
            an.data("myCrbLink", ak);
            var am = e('a.bibr[rid="' + ao + '"]');
            ak.data({ myCrb: ai, expandedHeight: al, myBodyLinks: am })
        });
        ad.each(function () {
            var ak = e(this);
            ak.removeClass("expanded").removeClass("highlight").addClass("two_line");
            ak.data("isCollapsed", true);
            if (x == -1) {
                x = ak.height();
                p = ak.outerHeight(true)
            }
        });
        var ag = U.height();
        var aj = ad.length;
        var ab = Math.floor(ag / p);
        var aa = Math.min(r, ab);
        if (aj > aa) {
            var Z = ai.height();
            var af = e("<a href='' class='seemore'>See more ...</a>");
            ai.find("div.portlet_content").append(af);
            af.click(t);
            var W = ai.height();
            var V = ai.height() - Z; var ae = Math.max(0, Math.floor((ag - V) / p));
            for (var Y = ae; Y < aj; Y++) {
                e(ad[Y]).hide().data("isExtra", true)
            }
            if (ae <= 0) {
                af.text("See links ...");
            }
            ...
            var X = ai.height();
            ai.data({ expandedHeight: W, collapsedHeight: X, collapsedNumLinks: ae })
        }
        ai.data({ isCollapsed: true, autoCollapseTimerId: -1 });
        var T = ai.css("width");
        ai.css({ position: "absolute", visibility: "visible", width: T, top: ac });
        if (ac <= P) {
            ai.hide();
        }
        ...
        var W = V.data("myCrb");
        W.addClass("stretched");
        W.css("z-index", 1); c(W);
        if (T && V.data("isExtra") && W.data("isCollapsed")) {
            H(W)
        } var U = V.data("expandedHeight");
        V.data("isCollapsed", false);
        V.animate({ height: U }, { complete: function () { W.css({ height: "auto" }) }, duration: "fast" });
        V.removeClass("two_line").addClass("expanded");
        s(V)
    } function a(U) {
        var T = e(this);
        y(T)
